...
const generateSavingsData = (month: Date) => {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    if (Math.random() > 0.7) {
      data.push({
        date: format(new Date(month.getFullYear(), month.getMonth(), i), "yyyy-MM-dd"),
        amount: Math.floor(Math.random() * 15000) + 5000,
        type: Math.random() > 0.5 ? "Tax Exemption" : "Investment",
...
export default function Dashboard() {
  ...
  const CustomCalendarEvent = ({ event }) => (
    <div className="flex items-center gap-2 p-2 bg-emerald-500/20 rounded-md backdrop-blur-sm">
        ...
        <Tabs defaultValue="income" className="space-y-12 py-[0px]">
          ...
          <TabsContent value="income" className="space-y-8">
            ...
            >
              ...
              <motion.div 
                variants={itemVariants}
                className="calendar-wrapper bg-gray-800 rounded-lg p-4 border border-gray-700"
              >
                <Calendar
                  localizer={localizer}
